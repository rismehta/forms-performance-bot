...
// See AEM Forms documentation for best practices.`,
  ...
  applyFixesToCurrentPR = async (suggestions, octokit, owner, repo, currentBranch) => {
    ...
    try {
      ...
      // Create commit with bot-identifiable message prefix (for clarity in git history)
      ...
      // Group files by impact to avoid duplication
      const impactGroups = {};
      filesChanged.forEach(f => {
        if (!impactGroups[f.impact]) {
          impactGroups[f.impact] = [];
        }
        impactGroups[f.impact].push(f.filePath);
      });
      
      const impactSummary = Object.entries(impactGroups)
        .map(([impact, files]) => `- ${impact} (${files.length} file${files.length > 1 ? 's' : ''})`)
        .join('\n');
    ...
    if (fix.type === 'css-import-fix') {
      ...
      if (fix.fixedCSSCode) {
        ...
        if (hasInlinedImports && !fix.fixedCSSCode.includes('/* Inlined from:')) {
          ...
          // Check for both 'background-image' and 'background:' (shorthand)
          const lineHasBackgroundImage = lineIndex >= 0 && lineIndex < lines.length && 
                                          (lines[lineIndex].includes('background-image') || 
                                           (lines[lineIndex].includes('background') && lines[lineIndex].includes('url(')));
          
          if (lineHasBackgroundImage) {
